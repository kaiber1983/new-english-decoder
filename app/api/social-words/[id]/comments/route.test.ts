...
describe("/api/social-words/[id]/comments", () => {
  beforeEach(() => {
    vi.useFakeTimers();
  ...
  it("adds a comment to a shared word", async () => {
    ...
    const response = await POST(
      new Request("http://localhost", {
        method: "POST",
        body: JSON.stringify({ comment: "I got it." }),
      }),
      {
    ...
    expect(updated.comments).toEqual(["I got it."]);
  });
});
